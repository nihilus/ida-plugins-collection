  ...
  switch ( event )
  {
    case hxe_right_click:
      { // If the current item is an if-statement, then add the menu item
		...
		// 不处理变量声明, 需要匹配字符串获得变量索引
		if ((vu.item.citype == VDI_EXPR && vu.item.e->op == cot_var)) 
		{
			add_custom_viewer_popup_item(vu.ct, "map var to...", "", map_var_to, &vu);
		}
		// 如果当前变量可以反映射,则显示相应菜单
		if (is_var_can_unmap(vu)) 
		{
			add_custom_viewer_popup_item(vu.ct, "unmap var from...", "", ummap_var_from, &vu);
		}
		// 隐藏当前行
		if (is_current_line_can_hide(vu)) 
		{
			add_custom_viewer_popup_item(vu.ct, "hide this line", "", hide_this_line, &vu);
		}
      }
      break;
    case hxe_maturity:
      { // If the ctree is ready, invert marked ifs
        ...
        if ( new_maturity == CMAT_FINAL ) // ctree is ready
		{
			...
			safe_convert(cfunc);
			// 修改变量显示名
			change_var_name(cfunc);
			// 隐藏行
			hide_if_asg_equal_var(cfunc);
		}
      }
      break;
  }
...
void idaapi run(int)
{
	point_null_run();	// 设置是否将0转成NULL
}

//--------------------------------------------------------------------------
static char comment[] = "Hex-Rays helper";
...
plugin_t PLUGIN =
{
  ...
  comment,              // long comment about the plugin
                        ...
                        // or as a hint
  "hello thinkSJ",                   // multiline help about the plugin
  "Hex-Rays helper", // the preferred short name of the plugin
